...
async function getSvaDestaque() {
  ...
  const data: { sva: Svas[], configuracoes: {assine_ja: string} } = await request(endpoint, query);
      ...
      <div className="py-8 px-4 max-w-screen-xl lg:py-10 grid lg:grid-cols-[400px_minmax(900px,_1fr)_100px] mx-auto space-y-4 lg:space-y-0">
        <div className="lg:max-w-[80%] w-full space-y-2">
          ...
          <p className="text-md text-center md:text-start">
            {" "}
            Aqui na MX Fibra, você monta o seu plano: escolha sua velocidade e
            adicione os melhores aplicativos de streaming!
          </p>
        </div>
          ...
          {data.sva.map((sva, index) => {
            ...
            return (
              ...
              >
                ...
                <Link href={`${data?.configuracoes?.assine_ja}&text=Gostaria de assinar o plano ${sva.nome}` || ""}>
