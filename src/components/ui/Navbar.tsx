...
const MobileMenu = ({
  ...
  return (
    <div
      className={`mobile-menu ${isOpen ? "open" : ""} lg:hidden space-y-1 ${
        inter.className
      } `}
    >
      <Link
        href={`/${cidade.slug}/para-voce`}
        className={`${location.includes("para-voce") ? "current" : ""}`}
      >
        Para Você
      </Link>
      <Link href={`/${cidade.slug}/para-empresa`}>Para Empresa</Link>
      ...
      {cidade?.nome ? (
        <Link href={"/"}>
          <button className="flex hover:text-blue-500 cursor-pointer text-md ">
            MX Fibra em{" "}
            <span className="ml-1 text-mx-blue-500">
    ...
    <>
      <div className="main-navbar select-none">
        <div className="navbar">
          <div
            className={`navbar__bottom text-lg md:text-md lg:text-xs xl:text-xs`}
          >
                  ...
                  <div className="flex items-center gap-10 ml-5">
                    <div className="bottom__left-options">
                      <Link
                        href={`/${cidade.slug}/para-voce`}
                        className={`${
                          location.includes("para-voce") ? "current" : ""
                        }`}
                      >
                        Para Você
                      </Link>
                      <Link
                        href={`/${cidade.slug}/para-empresa`}
                        className={`${
                          location.includes("para-empresa") ? "current" : ""
                        }`}
                      >
                        Para Empresa
                      </Link>
